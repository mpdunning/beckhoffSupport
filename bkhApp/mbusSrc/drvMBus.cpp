 ...
 *---------------------------------------------------------------------------*/
  epicsMessageQueue* pmq; static msgq_t msgq; int j,k,stat;
 ...
 *---------------------------------------------------------------------------*/
  ...
  int st=0; epicsUInt16* pw=(epicsUInt16*)iodone.data;
 ...
 *---------------------------------------------------------------------------*/
  static iodone_t iodone;
  epicsUInt16 cbe,w; int st,woff=0x800,maddr;
 ...
 *---------------------------------------------------------------------------*/
  static iodone_t iodone; int st; epicsUInt16* pw=(epicsUInt16*)&msgq.d;
 ...
 *---------------------------------------------------------------------------*/
  int wfunc=MODBUS_WRITE_SINGLE_REGISTER; int st;
  int rfunc=MODBUS_READ_HOLDING_REGISTERS;
  int woff=0x800,maddr; epicsUInt16 cb,cbe;
  maddr=msgq.maddr+woff; cb=0x80+r; *v=0;
  st=doModbusIO( _pmb,_pmb->slave,rfunc,maddr,&cbe,1);
  if(st) return(st);
